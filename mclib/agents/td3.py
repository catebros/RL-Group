...
class TD3Agent:
    """Stable-Baselines3 TD3 wrapper with the same outer shape as mclib agents."""
    ...
    def build(self):
        if self.env_factory is None:
            raise ValueError("TD3Agent requires an env_factory before it can build a model.")
    ...
    def train(
        self,
        run_name,
        total_timesteps=S4_TD3_TOTAL_TIMESTEPS,
    ...
    def predict(self, obs, deterministic=True, **kwargs):
        if self.model is None:
            raise ValueError("TD3Agent has no model. Build, train, or load it first.")
    ...
    def evaluate(
        ...
        if eval_factory is None:
            raise ValueError("TD3Agent.evaluate requires an env_factory.")
    ...
    def save(self, path):
        if self.model is None:
            raise ValueError("TD3Agent has no model to save.")
